...
begin
	...
	for j in (select uiid from SystemUser@server1 where UserName = name) loop
		cd:=1;
	end loop;
	if cd=1 then
		select uiid into luid from SystemUser@server1 where UserName = name;
	
	else 
		raise no_seller_found_site;
	end if;
	for k in (select miid from Medicine@server1 where MedicineName = mediname) loop
		dd:=1;
	end loop;
	for l in (select Stock from Inventory@server1 where Stock>selquantity) loop
		ee:=1;
	end loop;
	if ee=0 then
	...
	if dd=1 then
		select miid into lmid from Medicine@server1 where MedicineName = mediname;
	else 
		raise no_medicine_found_site;
	end if; 
	if(cd=1 and dd=1 and ee=1) then
		insert into SellingDetails@server1 values(30,luid,lmid,selcname,LOCALTIMESTAMP,selquantity,seltotalprice);
		select Iid,Stock into iid,newstock from Inventory@server1 where Miid = lmid;  
		--newstock := newstock-selquantity;
		--update Inventory@server1 set Stock = newstock where Iid=iid;
	...
	exception
		when no_medicine_found_site then
			dbms_output.put_line('Medicine Not Found in server');
		when no_seller_found_site then
			dbms_output.put_line('Unknown seller in server');
		when stock_out_site then
			dbms_output.put_line('Not Enough Quantity In stock in server');
commit;
